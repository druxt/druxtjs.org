...
const { HOST, emailHash, gravatarUrl, normalise } = (await import('../nuxt/lib/gravatar.js')).default
...
describe('emailHash', () => {
  ...
  it('is null without an address, and where the browser cannot hash', async () => {
    ...
    assert.equal(await emailHash('editor@example.com', { digest: () => Promise.reject(new Error('no')) }), null)
  })
})
