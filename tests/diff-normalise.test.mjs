...
const doc = (children) => ({
  ...
  included: children.map((c) => ({ id: c.id, type: 'jsonapi_diff--diff', attributes: { fields: c.fields || {} } })),
})

const removed = (uuid, delta, fields) => ({ id: `${uuid}:${100 + delta}:`, meta: { field: 'field_content', left_delta: delta, right_delta: null, status: 'removed' }, fields })
const added = (uuid, delta, fields) => ({ id: `${uuid}::${200 + delta}`, meta: { field: 'field_content', left_delta: null, right_delta: delta, status: 'added' }, fields })
const textField = (side, value) => ({ field_text: { label: 'Text', status: side === 'left' ? 'removed' : 'added', left: side === 'left' ? value : '', right: side === 'right' ? value : '', ops: [] } })
...
describe('normaliseDiff re-pairs a rebuilt draft', () => {
  test('an edited paragraph becomes one changed block keyed on the added uuid', () => {
    const result = normaliseDiff(doc([
    ...
    assert.equal(changed[0].uuid, 'new-1', 'keyed on the rendered (added) uuid, not the removed one')
  ...
  test('a real removal with no added counterpart stays removed', () => {
    const result = normaliseDiff(doc([
      removed('old-0', 0, textField('left', 'a paragraph that was deleted from the draft')),
    ]))
    assert.equal(result.summary.removed, 1)
    assert.equal(result.summary.changed, 0)
  })
...
describe('removed-block placement', () => {
  const survivorText = (side) => textField(side, 'the config value is set in services.yml under cors')
  const changed = (side) => textField(side, side === 'left' ? 'old wording of the paragraph here' : 'new wording of the paragraph here')

  test('a deleted paragraph anchors after its surviving predecessor', () => {
    const result = normaliseDiff(doc([
      removed('old-0', 0, survivorText('left')),
      removed('gone-1', 1, textField('left', 'this whole paragraph is being deleted from the draft')),
    ...
    assert.equal(gone.placeAfter, 'new-0', 'anchored after the surviving predecessor (rendered uuid)')
    assert.equal(gone.placeBefore, undefined)
  })

  test('a paragraph deleted from the top anchors before its surviving successor', () => {
    const result = normaliseDiff(doc([
      removed('gone-0', 0, textField('left', 'a leading paragraph that the draft dropped entirely')),
...
describe('groupRuns', () => {
  test('reorders a change region to all removals then all additions', () => {
    const grouped = groupRuns([
      { type: '-', text: 'enables ' }, { type: '+', text: 'supplies ' },
      { type: '-', text: 'when ' }, { type: '+', text: 'settings' },
    ])
  ...
  test('keeps common runs as boundaries between regions', () => {
    const grouped = groupRuns([
      { type: '=', text: 'the ' }, { type: '-', text: 'old' }, { type: '+', text: 'new' },
      { type: '=', text: ' word' },
    ])
    assert.deepEqual(grouped.map((r) => r.type), ['=', '-', '+', '='])
  })
})

describe('condenseRuns', () => {
  test('trims a long trailing unchanged run to leading context with an ellipsis', () => {
    const tail = 'a very long unchanged trailing paragraph that should be cut down to only its first words for the panel'
    const [change, same] = condenseRuns([{ type: '+', text: 'added' }, { type: '=', text: tail }], 30)
