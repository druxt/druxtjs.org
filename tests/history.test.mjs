...
describe('parseLog', () => {
  test('reads each commit, newest first, with the path the page had then', () => {
    const commits = parseLog(gitLog(
      [sha('b'), '1667435566', 'chore(docs): add devtools guide page', 'docs/nuxt/content/guide/proxy.md'],
      [sha('a'), '1636498122', 'feat(#362): add Proxy support (#364)', 'docs/content/guide/proxy.md'],
    ))
    assert.deepEqual(commits, [
      { sha: sha('b'), date: '2022-11-03T00:32:46+00:00', subject: 'chore(docs): add devtools guide page', path: 'docs/nuxt/content/guide/proxy.md' },
      { sha: sha('a'), date: '2021-11-09T22:48:42+00:00', subject: 'feat(#362): add Proxy support (#364)', path: 'docs/content/guide/proxy.md' },
    ])
  })

  test('a subject that reads like a path is still the subject', () => {
    const [commit] = parseLog(gitLog([sha('c'), '1612141200', 'docs/guide/client.md: fix a typo', 'docs/guide/client.md']))
  ...
  test('refuses an entry it cannot read rather than skipping it', () => {
    assert.throws(() => parseLog('\x1enot-a-sha\x1f1612141200\x1fsubject\n\nfile.md\n'), /Unreadable git log entry/)
    assert.throws(() => parseLog(`\x1e${sha('d')}\x1f1612141200\x1fsubject\n`), /Unreadable git log entry/)
  })
})
...
describe('versions', () => {
  ...
  const contents = { [sha('1')]: 'A', [sha('2')]: 'B', [sha('3')]: 'B', [sha('4')]: 'A', [sha('5')]: 'C' }
  ...
  test('a commit that leaves the page as it was is not a version', () => {
    assert.deepEqual(versions(commits, read).map((version) => version.subject), ['write', 'change', 'change back', 'rewrite'])
  })

  test('a page that changes back is a new version, because it changed', () => {
    assert.deepEqual(versions(commits, read).map((version) => version.content), ['A', 'B', 'A', 'C'])
  })

  test('a commit at which the page does not exist is skipped', () => {
    const removed = { ...contents, [sha('4')]: null }
    assert.deepEqual(versions(commits, (commit) => removed[commit.sha]).map((version) => version.content), ['A', 'B', 'C'])
  })
})
