...
// Anonymous visitors go to the frontend. DRUXT_FRONTEND_URL wins; otherwise
// the frontend's own route is picked from this environment's routes.
//
// The public domain comes first. Matching `nuxt.` alone, as this did, only
// ever matched the autogenerated `nuxt.<branch>.<project>.<cluster>` host,
// so production sent readers to an `amazee.io` URL while its own domain sat
// in the same list. Production also no longer has an autogenerated route to
// match, so the domain is the answer rather than a preference.
$frontend = getenv('DRUXT_FRONTEND_URL');
if (!$frontend) {
  $host_of = static fn (string $route): string => (string) parse_url($route, PHP_URL_HOST);
  $named = static fn (string $host): ?string => current(array_filter($routes, static fn (string $route): bool => $host_of($route) === $host)) ?: NULL;
  // A `nuxt.`-labelled route last: on an environment with no custom domain
  // the autogenerated one is the only frontend there is.
  $labelled = current(array_filter($routes, static fn (string $route): bool => str_starts_with($host_of($route), 'nuxt.'))) ?: NULL;
  $frontend = $named('druxtjs.org') ?? $named('www.druxtjs.org') ?? $labelled;
}
// Production always has a frontend to name, whatever its routes say.
if (!$frontend && getenv('LAGOON_ENVIRONMENT_TYPE') === 'production') {
  $frontend = 'https://druxtjs.org';
}
if ($frontend) {
