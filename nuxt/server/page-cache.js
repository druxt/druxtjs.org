...
const createHandler =
  ...
  async (req, res) => {
    ...
    // Production's own autogenerated route serves the same pages as the
    // public domain; search engines must not index it under that host.
    const host = String(req.headers.host || '').replace(/:\d+$/, '').replace(/\.$/, '').toLowerCase()
    if (noindex || host.endsWith('.amazee.io')) {
