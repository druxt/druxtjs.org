...
/**
 * The origin a browser reaches Drupal on, for the OAuth authorize redirect.
 * The server's own base URL is an internal service name on Lagoon, so the
 * public route is taken instead: the configured override, else the `cms`
 * custom route, else the nginx service's autogenerated route on a preview.
 * Locally the browser and the server share the base URL.
 *
 * @param {object} env - The process environment.
...
const backendOrigin = (env) => {
  const origin =
    env.DRUXT_PUBLIC_URL ||
    serviceRoute(env.LAGOON_ROUTES, 'cms') ||
    serviceRoute(env.LAGOON_ROUTES, 'nginx') ||
    env.DRUXT_BASE_URL ||
    'http://127.0.0.1:8899'
  return origin.replace(/\/+$/, '')
}
